...
| `\oint` | `integral.cont` |
| `\cdot` | `dot.op` |
| `\left(\frac{\partial U}{\partial V}\right)_T` | `lr(( (partial U)/(partial V) ))_T` ⚠️ |
| `\underbrace{x}_{\text{foo}}` | `underbrace(x, "foo")` |
| `H^\circ` (estado estándar) | `H^circle.stroked.small` |

> ⚠️ **Paréntesis alrededor de una fracción.** `lr((A)/(B))` **no** dibuja paréntesis: Typst se los come
> como agrupación de la fracción y `lr()` se queda sin delimitadores que escalar. Hace falta un par
> interno explícito: `lr(( (A)/(B) ))`. La forma sin él pasó desapercibida en 52 derivadas parciales de
> 12 archivos hasta que se corrigió — no da error, simplemente renderiza sin paréntesis.
>
> No se puede chequear con grep, porque la forma correcta contiene a la incorrecta como subcadena.
> Hay un script que empareja paréntesis de verdad y sale con código 1 si encuentra algo (sirve para
> pre-commit):
> ```
> python3 scripts/check-lr-parens.py
> ```
...
Las **fórmulas** se compilan con Typst; los **diagramas** (curvas, ejes, esquemas) los genera
`scripts/make-diagrams.py`, que dibuja SVG calculando las curvas desde las ecuaciones reales
— gas ideal, Van der Waals reducida, construcción de Maxwell — en vez de trazarlas a ojo.

```
python3 scripts/make-diagrams.py     # regenera los 8 diagramas de termoquimica/
```

- Salida: `<curso>/<página>/fig_<página>_<nombre>.svg`, junto al HTML, igual que las fórmulas.
- El prefijo `fig_` los distingue de los SVG de Typst (`<página>_<N>.svg`), que están numerados
  por página y **se renumeran** si se inserta una fórmula en medio de un `.typ`.
- Colores tomados de la paleta de `styles/main.css` (fondo transparente, texto `#e6edf3`).
- En el HTML van dentro de `<div class="figure">` con `.figure-scroll` y `.figure-cap`
  (pie de figura), no en `.formula-block`.
...
- Componentes: `.formula-block`, `.concept-card`, `.card-grid`, `.tip-box`, `.page-nav`, `.problem-block`, `.figure`.
- `.figure` — contenedor de diagramas: `.figure-scroll` (scroll horizontal en pantallas angostas) + `.figure-cap` (pie de figura).
